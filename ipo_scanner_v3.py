#!/usr/bin/env python3
"""
IPO SNIPER v3.2 – INSTITUTIONAL QUANT ENGINE (Mainboard + SME)
MERGED PRODUCTION EDITION:
  • Multi-strategy scraper with NSE/BSE/Chittorgarh/Investorgain fallback chain
  • Bayesian weight update (fixed missing function from v1)
  • Corrected regex in parse_via_raw_text_stream (</table> → </td> bug fixed)
  • Full Shariah Governance Matrix (Najash + Qabda + Barakah)
  • Monte Carlo allotment engine (50k runs, Wilson CI)
  • Kelly criterion + Syndicate optimisation
  • SQLite persistence + Telegram alerting
"""
...
VERSION        = "IPO-SNIPER-v3.2-MERGED-PRODUCTION"
...
# Base scoring weights (Bayesian engine may adjust per regime)
...
# ═══════════════════════════════════════════════════════════
# BAYESIAN WEIGHT UPDATE  (was missing in v1 – fixed)
# ═══════════════════════════════════════════════════════════
def bayesian_weight_update(df: pd.DataFrame) -> Dict[str, float]:
    """
    Adjust scoring weights based on current market regime.
    Logic:
      • Hyper-bull (avg_sub > 80): trust subscription signal more, GMP less
      • Bear / tepid (avg_sub < 15): trust GMP more, downgrade sub noise
      • Neutral: return base weights
    """
    ...
    if avg_sub > 80:
        # High participation regime → subscription signal dominates
        ...
        # Low participation → GMP & Shariah become more predictive
...
# ═══════════════════════════════════════════════════════════
# SCRAPING LAYER – Strategy 1: Chittorgarh HTML table
# ═══════════════════════════════════════════════════════════
_BROWSER_HEADERS = {
    "User-Agent":      (
...
# ─── Strategy 2 (fallback): raw text-stream pattern matching ───────────────
def parse_via_raw_text_stream(html_content: str, ipo_type: str) -> pd.DataFrame:
    """
    Emergency Text Engine: parse company names from unstructured HTML when
    the table parser fails (e.g. CloudFlare challenge pages or layout changes).
    BUG FIX vs v1: regex pattern used </table> instead of </td> – corrected.
    """
    # Pattern A: anchor links like /ipo/company-slug/
    links_discovered = re.findall(
        r'href=["\']/ipo/([^"\']+)/["\']>(.*?)</a>',
        html_content, re.IGNORECASE
    )

    # Pattern B: JSON-style company_name keys
    if not links_discovered:
        raw_names = re.findall(
            r'["\']?company_name["\']?\s*:\s*["\']([^"\']+)["\']',
            html_content, re.IGNORECASE
        )
        # Pattern C: table cells (BUG FIX – was </table>, now </td>)
        if not raw_names:
            raw_names = re.findall(
    ...
    SKIP_WORDS = {"company", "compare", "click here", "home", "mainboard", "sme",
                  "name", "issuer", "no records found"}
...
# ─── Strategy 1: Chittorgarh structured HTML table ─────────────────────────
def scrape_chittorgarh_table(url: str, ipo_type: str) -> pd.DataFrame:
    try:
        session = _make_session()
        # Warm the session with a homepage visit first
        ...
        if resp.status_code != 200:
            log.warning(f"Non-200 from Chittorgarh ({resp.status_code}), trying text-stream fallback.")
        ...
        CSS_SELECTORS = [
            ...
            "#ipo_table",
            "table",   # last resort – grab first non-trivial table
        ]
        ...
        if not table:
            log.warning("No usable HTML table found; falling back to text-stream engine.")
            return parse_via_raw_text_stream(resp.text, ipo_type)
        ...
        for idx, h in enumerate(headers_parsed):
            if any(k in h for k in ("company", "issuer", "name")):
                col_map.setdefault("symbol", idx)
            elif any(k in h for k in ("size", "cr", "amt")):
                col_map.setdefault("issue_size", idx)
            elif any(k in h for k in ("price", "band")):
                col_map.setdefault("price", idx)
            elif any(k in h for k in ("close", "date", "end")):
                col_map.setdefault("close_date", idx)
            elif any(k in h for k in ("lot", "shares")):
                col_map.setdefault("lot_size", idx)
            elif "gmp" in h:
                col_map.setdefault("gmp", idx)
            elif any(k in h for k in ("sub", "times")):
                col_map.setdefault("sub", idx)
        ...
        for row in rows[1:]:
            ...
            if not symbol or symbol.lower() in SKIP:
                continue
            ...
            if "issue_size" in col_map and len(cols) > col_map["issue_size"]:
                ...
                if m:
                    ...
                    if "cr" not in txt.lower() and issue_size > 500:
                        issue_size /= 100.0
            ...
            if "price" in col_map and len(cols) > col_map["price"]:
                ...
                if pm:
                    ...
                    if single:
                        price_lower = price_upper = float(single.group(1))
...
# ─── Strategy 3: Investorgain GMP page ─────────────────────────────────────
def scrape_investorgain_gmp() -> pd.DataFrame:
    """Fallback data source for live GMP data."""
...
# ═══════════════════════════════════════════════════════════
# FALLBACK CSV  (rich, realistic seed data)
# ═══════════════════════════════════════════════════════════
def ensure_fallback_csv():
    FALLBACK_CSV.parent.mkdir(parents=True, exist_ok=True)
    if FALLBACK_CSV.exists():
        return  # Don't overwrite user-edited fallback
...
# ═══════════════════════════════════════════════════════════
# UNIFIED FETCH ENGINE  (tries all 3 strategies in order)
# ═══════════════════════════════════════════════════════════
def fetch_unified_calendar() -> pd.DataFrame:
    """
    Fetch IPO data via a waterfall of strategies:
      1. Chittorgarh HTML scrape (SME + Mainboard pages)
      2. Investorgain GMP page
      3. Fallback CSV (seed/manual data)
    ...
    # Also try live subscription-status pages for richer GMP/sub data
    ...
    # Deduplicate by Symbol, keeping highest subscription record
...
def _enrich_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Add computed columns and ensure required fields exist."""
    today = datetime.today().date()
...
def monte_carlo_allotment_simulation(
    ...
    """
    Simulates retail allotment probability using binomial model.
    Returns (p_estimate, ci_lower, ci_upper).
    """
    ...
    retail_pool_inr   = issue_total_inr * 0.35     # SEBI: 35% retail quota
...
def build_syndicate_permutation_matrix(
    p_single: float, max_accounts: int = MAX_SYNDICATE
) -> Dict[int, float]:
    """P(at least one allotment) = 1 − (1 − p)^k for k accounts."""
    return {
        k: round(1.0 - math.pow(max(0.0, 1.0 - p_single), k), 6)
...
def optimal_syndicate_by_ev(
    ...
    opportunity_cost: float = 500.0,
) -> int:
    """Find k that maximises expected value after opportunity cost."""
...
def compute_full_allotment_profile(row: pd.Series) -> AllotmentProfile:
    ...
    b_odds      = gmp_gain / max(1.0, 1500.0)   # risk-normalised odds
    ...
    return AllotmentProfile(
        symbol=symbol,
        p_single_hypergeom=0.0,          # placeholder (hypergeom not needed given MC)
...
def run_shariah_screen(row: pd.Series) -> ShariahVerdict:
    """
    Three-framework evaluation:

    1. Ala Hazrat Framework
       Detects artificial demand inflation (Najash) when GMP > 40 % AND
       subscription > 80×. Both signals together indicate speculative
       price manipulation disconnected from asset fundamentals.

    2. Shaykh Nurjan Framework
       Computes Barakah Index. Microcap issues (< ₹20 Cr) carry higher
       liquidity hazard and reduced capital safety → barakah penalty.

    3. Mufti Salman Azhari Framework  (Qabda rule)
       Enforces constructive possession: shares must settle in Demat
       before any resale. Listing-day flip before T+2 settlement = gharar.
    """
    symbol  = str(row.get("Symbol", "UNKNOWN"))
    gmp     = float(row.get("GMP", 0.0))
    sub     = float(row.get("SubscriptionTimes", 0.0))
    ...
    # ── Frame 2b: SME extreme subscription (pump risk) ───────────
    if sector == "SME" and sub > 200:
        barakah -= 10
        issues.append("SME Hyper-Subscription Pump Risk (Sub > 200×)")

    # ── Frame 3: Qabda mandate (always enforced) ──────────────────
    qabda = (
        "MANDATORY QABDA GUARD: Shares must settle in Demat (T+2) "
        "before any resale. Listing-day flips before settlement = Gharar (forbidden)."
    )
    ...
    return ShariahVerdict(
        symbol=symbol, tier=tier, barakah_index=halal_score,
...
def run_ipo_screener_v3():
    ...
    for _, row in df.iterrows():
        ...
        score_results.append(
            compute_master_score(row, allot_profiles[sym],
                                 sentiment_profiles[sym], shariah_verdicts[sym], weights)
        )
    ...
    for _, row in ranked.iterrows():
        ...
        print(
            f"  {sym:<28} {row['FinalScore']:>6.1f}  {row['Verdict']:<14}  "
            f"{row['SubscriptionTimes']:>6.1f}x  {row['gmp_pct']:>5.1f}%  "
            f"{ap.optimal_syndicate_size:>4}  {sh.tier}"
        )

        msg = (
            f"<b>{sym}</b> ➜ {row['Verdict']} ({row['FinalScore']}/100)\n"
            f"   📊 Sub: {row['SubscriptionTimes']:.1f}x | GMP: {row['gmp_pct']:.1f}%\n"
            f"   🎲 Syndicate: {ap.optimal_syndicate_size} PANs → "
            f"P(allot)={ap.p_single_monte_carlo*100:.3f}% "
            f"[95% CI: {ap.confidence_interval_95[0]*100:.2f}–{ap.confidence_interval_95[1]*100:.2f}%]\n"
    ...
    print(f"{'═'*70}\n")
    log.info("🏁 IPO Sniper v3.2 complete.")
    return df   # return for programmatic use / testing
