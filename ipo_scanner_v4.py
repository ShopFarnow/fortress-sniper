...
====================================
- Fetch: NSE API + Playwright (Chittorgarh AJAX) + Fallback CSV
- Quant: Monte Carlo allotment, Kelly, syndicate optimizer
- Shariah: Najash, Qabda, Barakah index
- Output: SQLite DB + Telegram alerts
"""
...
# Optional: Playwright (will be skipped if not installed)
try:
...
VERSION = "IPO-SNIPER-v4.0-PRODUCTION"
...
# Base weights (Bayesian will adjust)
BASE_WEIGHTS = {
...
# ═══════════════════════════════════════════════════════════
# FETCH ENGINE v4 (NSE API + Playwright + Fallback)
# ═══════════════════════════════════════════════════════════
...
def _nse_warmup(sess: requests.Session):
    """Warm NSE session cookies."""
    for url in ["https://www.nseindia.com", "https://www.nseindia.com/market-data/upcoming-issues-ipo"]:
        try:
...
def _parse_nse_ipo_row(item: dict, sector: str) -> Optional[dict]:
    ...
    size = float(re.search(r"[\d.]+", size_raw).group()) if re.search(r"[\d.]+", size_raw) else 50.0
    if size > 50000:
        size /= 1e7
    # Lot size
    lot_raw = str(item.get("lotSize", item.get("minBidQuantity", "1000" if sector=="SME" else "50")))
    lot = int(re.search(r"\d+", lot_raw).group()) if re.search(r"\d+", lot_raw) else (1000 if sector=="SME" else 50)
    # Subscription
    sub_raw = str(item.get("subscriptionTimes", "0"))
    sub = float(re.search(r"[\d.]+", sub_raw).group()) if re.search(r"[\d.]+", sub_raw) else 0.0
    # Close date
    ...
    return {
        ...
        "DaysToClose": max(0, (close_date - today).days),
        "Source": "nse_api",
    }
...
def fetch_nse_ipos() -> pd.DataFrame:
    ...
    for ep in NSE_IPO_ENDPOINTS:
        try:
            ...
            log.warning(f"NSE API error for {ep}: {e}")
    if all_rows:
        log.info(f"✅ NSE API: {len(all_rows)} IPOs fetched")
        return pd.DataFrame(all_rows)
    return pd.DataFrame()

# --- Strategy B: Chittorgarh (AJAX probe + Playwright) ---
CHITTORGARH_URLS = {
...
# --- Master fetch function (drop-in for v3) ---
def fetch_ipo_calendar(use_playwright: bool = True) -> pd.DataFrame:
    ...
    for ipo_type, url in CHITTORGARH_URLS.items():
        log.info(f"  Chittorgarh {ipo_type}...")
